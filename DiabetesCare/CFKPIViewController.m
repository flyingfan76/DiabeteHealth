...
@interface CFKPIViewController () <TKCalendarDayViewDelegate>

@end

@implementation CFKPIViewController{
...
- (void)viewDidLoad
{
    ...
    NSString *htmlFile = [[NSBundle mainBundle] pathForResource:@"KPIPage" ofType:@"html"];
...
- (void)configureCell:(CFKPIGlycemiaTableCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.myBloodSugar.text = [info.value stringValue];
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    static NSString *CellIdentifier = @"cfKPISceneBloodSugar";
    
    CFKPIGlycemiaTableCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell == nil) {
        cell = [[CFKPIGlycemiaTableCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier] ;
    }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    CFGlycemiaModifyViewController *destinationController = [segue destinationViewController];
...
-(NSFetchedResultsController *)fetchedResultsController {
    ...
    [fetchRequestItems setPredicate:[NSPredicate predicateWithFormat:@"toMaster == %@ and (type == 0 or type == 1) and date >= %@ and date < %@",myUser,self.workingDate,tomorrow]];
...
- (void)controller:(NSFetchedResultsController *)controller didChangeObject:(id)anObject atIndexPath:(NSIndexPath *)indexPath forChangeType:(NSFetchedResultsChangeType)type newIndexPath:(NSIndexPath *)newIndexPath {
    ...
    switch(type) {
        ...
        case NSFetchedResultsChangeUpdate:
            [self configureCell:(CFKPIGlycemiaTableCell *)[tableView cellForRowAtIndexPath:indexPath] atIndexPath:indexPath];
            break;
...
- (void)controllerDidChangeContent:(NSFetchedResultsController *)controller {
    ...
    [self updateWebView];
    [self updateHistoryViewForAfter];
    [self updateHistoryViewForBefore];
...
- (void)updateHistoryViewForBefore{
    ...
    //out of data store based on some paramator
    [fetchRequestItems setPredicate:[NSPredicate predicateWithFormat:@"toMaster == %@ and type == 0 and date == %@",myUser,self.workingDate]];
    ...
    if (fetchedObjects){
        ...
        if ( [fetchedObjects count] == 0 ){
            ...
            historyAccount.type = [NSNumber numberWithInt:0];
...
- (void)updateHistoryViewForAfter{
    ...
    //out of data store based on some paramator
    [fetchRequestItems setPredicate:[NSPredicate predicateWithFormat:@"toMaster == %@ and type == 1 and date == %@",myUser,self.workingDate]];
    ...
    if (fetchedObjects){
        ...
        if ( [fetchedObjects count] == 0 ){
            historyAccount = [NSEntityDescription insertNewObjectForEntityForName:@"HistoryRecords" inManagedObjectContext:globalcontext];
            historyAccount.toMaster = myUser;
            historyAccount.type = [NSNumber numberWithInt:1];
            historyAccount.date = self.workingDate;
        ...
        for (int i = 0; i<[allPoints count]; i++) {
            TodayRecord *o = [allPoints objectAtIndex:i];
            if ( [o.type isEqual:[NSNumber numberWithInt:1]] ){
                total = [NSNumber numberWithFloat:([total floatValue] + [o.value floatValue])];
                countOfObjects++;
            }
        }
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    ...
    if ([[sectionInfo name] caseInsensitiveCompare:@"0"]==NSOrderedSame){
        return NSLocalizedString(@"Before food", nil);
    }else{
        return NSLocalizedString(@"After food", nil);
    }
